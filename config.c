...
void InitMotor1EPwmGpio(void)
{
	...
	// set up EPWM-B
    GpioCtrlRegs.GPAPUD.bit.GPIO8 = 1;    // Disable pull-up on GPIO2 (EPWM2A)
    GpioCtrlRegs.GPAPUD.bit.GPIO9 = 1;    // Disable pull-up on GPIO3 (EPWM2B)
    GpioCtrlRegs.GPAMUX1.bit.GPIO8 = 1;   // Configure GPIO2 as EPWM2A
    GpioCtrlRegs.GPAMUX1.bit.GPIO9 = 1;   // Configure GPIO3 as EPWM2B

    // set up EPWM-C
    GpioCtrlRegs.GPAPUD.bit.GPIO10 = 1;    // Disable pull-up on GPIO2 (EPWM2A)
...
void InitMotor1_DRV_Gpio(void)
{
	...
	GPIO_WritePin(MOTOR1_EN_GATE_GPIO, 1);  // Disable EN_GATE
...
void PWM_1ch_UpDwnCnt_CNF(volatile struct EPWM_REGS * ePWM, Uint16 period, int16 db)
{
	...
	ePWM->TBCTR = 0;
	ePWM->TBCTL.bit.CTRMODE   = TB_COUNT_UPDOWN; //TB_COUNT_UP;//
	...
	ePWM->CMPCTL.bit.SHDWAMODE = CC_SHADOW;//CC_IMMEDIATE;
//	ePWM->CMPCTL.bit.SHDWBMODE = CC_SHADOW;//CC_IMMEDIATE;
	ePWM->CMPCTL.bit.LOADAMODE = CC_CTR_ZERO;//CC_CTR_ZERO_PRD;
//	ePWM->CMPCTL.bit.LOADBMODE = CC_CTR_ZERO;//CC_CTR_ZERO_PRD;

	// Action Qualifier SubModule Registers
	ePWM->AQCTLA.bit.CAU = AQ_CLEAR;//AQ_SET;  //
	ePWM->AQCTLA.bit.CAD = AQ_SET;  // AQ_SET;

//	ePWM->AQCTLA.bit.CBU = AQ_CLEAR;
//	ePWM->AQCTLA.bit.CBD = AQ_SET;
...
void PWM_1ch_UpDwnCnt_CNF_noDB(volatile struct EPWM_REGS * ePWM, Uint16 period)
{
	...
	ePWM->TBPHS.bit.TBPHS = 0;
	ePWM->TBCTR = 0;
	ePWM->TBCTL.bit.PHSDIR    = 1;              // 0:down / 1:up after sync
	...
	ePWM->CMPCTL.bit.LOADAMODE = CC_CTR_ZERO_PRD;
...
void InitSpiRegs_DRV830x(volatile struct SPI_REGS *s)
{
	...
	s->SPICTL.bit.CLK_PHASE = 1;    	// Rising edge without delay
	s->SPICTL.bit.OVERRUNINTENA = 0;	// disable reciever overrun interrupt

	s->SPIBRR.bit.SPI_BIT_RATE = 3;		// SPICLK = LSPCLK / 4 (max SPICLK)

	s->SPIFFCT.bit.TXDLY = 0;
